...
def test_compute_utility_uses_reference_prices_when_provided():
...
def test_simulate_portfolio_delta_does_not_mutate_original():
    economics = AgentEconomics()
    portfolio = {"credits": 10.0, "gold": 2.0}

    simulated = economics.simulate_portfolio_delta(portfolio, {"gold": -1.0, "corn": 4.0})

    assert portfolio == {"credits": 10.0, "gold": 2.0}
    assert simulated == {"credits": 10.0, "gold": 1.0, "corn": 4.0}


def test_evaluate_trade_utility_returns_positive_delta_for_favorable_trade():
    economics = AgentEconomics()
...
def test_evaluate_order_utility_handles_bid_and_ask():
    economics = AgentEconomics()
    ...
    assert bid_delta == 2.0
    assert ask_delta == 2.0


def test_evaluate_order_utility_rejects_invalid_side():
    economics = AgentEconomics()
